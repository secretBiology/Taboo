package com.secretbiology.taboo.constants;

public class GameVariables {

    public static final String TIME_PER_ROUND = "timePerRound";
    public static final int DEFAULT_TIME_PER_ROUND = 60;

    public static final String NUMBER_OF_ROUNDS = "numberOfRounds";
    public static final int DEFAULT_NUMBER_OF_ROUNDS = 5;

    public static final String CURRENT_WORD = "currentWord";
    public static final int DEFAULT_CURRENT_WORD= 0;

    public static final int CORRECT_ANSWER_DEFAULT = 10;
    public static final String CORRECT_ANSWER = "correctAns";
    public static final int SKIP_DEFAULT = -3;
    public static final String SKIP = "SkipAns";
    public static final int TABOO_DEFAULT = -5;
    public static final String TABOO = "tabooAns";

    public static final String TEAM_RED_POINTS = "teamRed_points";
    public static final String TEAM_BLUE_POINTS = "teamBlue_points";

    public static final String RED_CORRECT = "redCorrect";
    public static final String RED_SKIP = "redSkip";
    public static final String RED_TABOO = "redTaboo";

    public static final String BLUE_CORRECT = "blueCorrect";
    public static final String BLUE_SKIP = "blueSkip";
    public static final String BLUE_TABOO = "blueTaboo";

}
